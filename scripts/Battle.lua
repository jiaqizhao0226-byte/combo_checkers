...
function Battle.GenerateTestLevel_Boss_CoralGuardian(state)
    ...
    -- 第三章小怪
    placeMinionsForTest(state, { "coral_snapper", "sea_urchin", "reef_starfish", "coral_vine", "splitting_urchin" }, 8, 3, usedPositions)
...
function Battle.GenerateLevel(state, level)
    ...
    -- 黎明使者: 冒险开始时重置免死标记（整次冒险仅一次）
    if Skills.Level(state.skills, "dawn_herald") >= 1 then
        state.hero._dawnUsed = false
    end
    ...
    if isBoss then
        ...
        elseif chapter == 3 then
            bossChapterEnemies = { "coral_snapper", "sea_urchin", "reef_starfish", "coral_vine", "splitting_urchin" }
        else
    ...
    elseif chapter == 4 then
        ...
        elseif chapter == 3 then
            -- 第三章: 珊瑚系敌人，后期加入特殊机制怪
            enemyTypes = { "coral_snapper", "coral_snapper", "sea_urchin", "coral_vine" }
            if stageInChapter >= 4 then
                enemyTypes = { "coral_snapper", "sea_urchin", "reef_starfish", "spine_anemone", "coral_vine", "splitting_urchin" }
            end
            if stageInChapter >= 7 then
                enemyTypes = { "coral_snapper", "sea_urchin", "reef_starfish", "spine_anemone", "coral_priest", "coral_vine", "splitting_urchin" }
            end
        else
        ...
        elseif chapter == 3 then
            ...
            state.rescueCount = 0
            -- 救援目标: 2~4只寄居蟹，随关卡递增
            local crabCount = math.min(2 + math.floor(stageInChapter / 3), 4)
            ...
            for ci = 1, crabCount do
                ...
                if crabCol and shellCol and crabCol < shellCol then
                    ...
                    local obstCount = math.max(1, math.floor(pathLen * 0.5))
                    local pathCols = {}
                    for c = crabCol + 1, shellCol - 1 do
                        if HexGrid.InBounds(c, row) and not usedPositions[c..","..row] then
                            pathCols[#pathCols + 1] = c
                    ...
                    for oi = 1, math.min(obstCount, #pathCols) do
                        local oc = pathCols[oi]
                        claimPos(oc, row)
                        HexGrid.AddObstacle(board, oc, row)
                    end
...
Battle.ENDLESS_ENEMY_POOLS = {
    ...
    [4] = { "vortex_eel", "fire_sprite", "lava_giant", "fission_flame",
            "coral_snapper", "sea_urchin", "ghost_shark", "coral_vine", "splitting_urchin",
            "swift_barracuda", "charm_jelly" },
    -- Wave 20+: 全部最强怪
    ...
    if chapter == 3 and stageInChapter % 3 == 1 then
        ...
        for ci = 1, crabCount do
            ...
            if crabCol and shellCol and crabCol < shellCol then
                ...
                local obstCount = math.max(1, math.floor(#pathCols * 0.5))
                for i = #pathCols, 2, -1 do
                    local j = math.random(1, i)
                    pathCols[i], pathCols[j] = pathCols[j], pathCols[i]
                end
...
function Battle.TrySpawnEnemies(state)
    ...
    elseif chapter == 3 then
        enemyTypes = { "coral_snapper", "sea_urchin", "coral_vine" }
        if stageInChapter >= 4 then
            enemyTypes = { "coral_snapper", "sea_urchin", "spine_anemone", "coral_vine", "splitting_urchin" }
        end
        if stageInChapter >= 7 then
            enemyTypes = { "coral_snapper", "sea_urchin", "spine_anemone", "coral_priest", "coral_vine", "splitting_urchin" }
        end
    else
...
function Battle.ExecuteJump(state, jumpInfo, isLastStep)
    ...
    if shieldLv >= 1 then
        local perStack = 5 + shieldLv * 3     -- Lv1=8, Lv2=11, ..., Lv5=20
        local shieldAmt = perStack
        -- Lv5: 5+连击护盾翻倍
        if shieldLv >= 5 and state.combo >= 5 then
            shieldAmt = shieldAmt * 2
        end
        -- 连击护盾上限: 50% 最大生命值（Lv5时为75%）
        local shieldCapPct = shieldLv >= 5 and 0.75 or 0.50
        local shieldCap = math.floor(state.hero.maxHp * shieldCapPct)
...
function Battle.HandleEnemyDeath(state, enemy, fromChain, skipShockwave, skipDeathSFX)
    ...
    local deathColors = {
        ...
        coral_vine = {60, 200, 100}, splitting_urchin = {200, 100, 220},
    }
    ...
    -- === 珊瑚藤蔓: 死亡时清除所有自己生长出的藤蔓障碍 ===
    if enemy.enemyType == "coral_vine" and enemy._vineObstacles and #enemy._vineObstacles > 0 then
        local board = state.board
        local removed = 0
        for _, vineObs in ipairs(enemy._vineObstacles) do
            for i = #board.obstacles, 1, -1 do
                local obs = board.obstacles[i]
                if obs == vineObs then
                    table.remove(board.obstacles, i)
                    removed = removed + 1
                    Battle.AddVFX(state, "death_puff", {
                        col = obs.col, row = obs.row, duration = 0.4,
                        enemyColor = {60, 200, 100},
                    })
                    break
                end
            end
        ...
        if removed > 0 then
            Battle.AddFloatingText(state, enemy.col, enemy.row,
                "🌿藤蔓消散", {60, 200, 100, 255})
            Battle.AddLog(state, string.format("珊瑚藤蔓死亡，%d 块藤蔓障碍消失！", removed))
        end
    end
...
Battle.COMBO_REWARDS = {
    ...
    [5] = { name = "生命虹吸", icon = "combo_tier4", desc = "吸取全体敌人50%当前生命(Boss固定80)，等量恢复自身血量，溢出转虹吸护盾(上限60)" },
...
function Battle.ExecuteComboReward(state, threshold)
    ...
    elseif threshold == 5 then
        -- === 5连: 生命虹吸 ===
        -- 吸取全体敌人生命（小怪50%当前HP，Boss固定80），治疗英雄
        ...
        for _, e in ipairs(enemies) do
            if e.hp > 0 then
                ...
                if e.isBoss then
                    ...
                    drainDmg = math.floor(e.hp * 0.5)
                    drainDmg = math.max(drainDmg, 10)  -- 最少吸10
...
function Battle.EnemyAct(state, enemy)
    ...
    -- === 珊瑚藤蔓: 静止不动，每回合生长一块障碍 ===
    if enemy.enemyType == "coral_vine" then
        return Battle.CoralVineAct(state, enemy)
    end
...
--- 珊瑚藤蔓 AI：静止不动；每回合往相邻随机空格生长一块藤蔓障碍；若有攻击范围内可攻击目标则攻击
function Battle.CoralVineAct(state, enemy)
    local board = state.board
    local hero = state.hero

    -- 初始化藤蔓障碍追踪表（每个实例独立）
    if not enemy._vineObstacles then enemy._vineObstacles = {} end

    -- 收集所有相邻空格（6方向）
    local neighbors = HexGrid.GetNeighbors(enemy.col, enemy.row)
    local emptyNeighbors = {}
    for _, nb in ipairs(neighbors) do
        local piece = HexGrid.GetPieceAt(board, nb.col, nb.row)
        if not piece then
            -- 确认没有障碍物
            local blocked = false
            for _, obs in ipairs(board.obstacles) do
                if obs.col == nb.col and obs.row == nb.row then blocked = true; break end
            end
            if not blocked then
                table.insert(emptyNeighbors, nb)
            end
        end
    end
    ...
    if #emptyNeighbors > 0 then
        local target = emptyNeighbors[math.random(1, #emptyNeighbors)]
        local newObs = { col = target.col, row = target.row, isVine = true, vineOwner = enemy }
        table.insert(board.obstacles, newObs)
        table.insert(enemy._vineObstacles, newObs)
        Battle.AddFloatingText(state, target.col, target.row,
            "🌿生长", {60, 200, 100, 255})
        Battle.AddLog(state, string.format("珊瑚藤蔓在 (%d,%d) 生长出障碍！", target.col, target.row))
    end

    -- 若英雄在攻击范围内则攻击（近战1格）
    local dist = HexGrid.CubeDistance(enemy.col, enemy.row, hero.col, hero.row)
    if dist <= (enemy.attackRange or 1) and enemy.atk > 0 then
        local actualDmg = math.max(1, enemy.atk - (hero.def or 0))
        if state.hasShield then
            actualDmg = math.floor(actualDmg / 2)
            state.hasShield = false
        ...
        if actualDmg > 0 then
            hero.hp = hero.hp - actualDmg
            Battle.AddFloatingText(state, hero.col, hero.row,
                "-" .. actualDmg, {255, 80, 60, 255})
        end
        Battle.AddLog(state, string.format("珊瑚藤蔓用藤刺攻击英雄，造成 %d 伤害", actualDmg))
        return { type = "attack", enemy = enemy, damage = actualDmg }
    end
...
function Battle.CoralPriestAct(state, enemy)
    ...
    local validMoves = HexGrid.FindValidMoves(state.board, enemy.col, enemy.row)
    if #validMoves > 0 then
        local fleeMove = Battle.FindFarthestMove(validMoves, hero.col, hero.row)
        if fleeMove then
            enemy.animFromCol = enemy.col
            enemy.animFromRow = enemy.row
            enemy.animTimer = 0.3
...
function Battle.CheckCrabRescue(state)
    local board = state.board
    if not board.crabs then return end
        ...
        if not crab.rescued and not crab.animTimer then
            ...
            for c = crab.col + 1, crab.shellCol - 1 do
                if HexGrid.InBounds(c, crab.row) then
                    ...
                    -- 检查是否有敌人挡路
                    if HexGrid.GetPieceAt(board, c, crab.row) then
                        pathClear = false
                        break
                    end
                end
            ...
            if pathClear then
                ...
                Battle.AddLog(state, string.format("🦀 寄居蟹从 (%d,%d) 开始奔向壳 (%d,%d)！",
                    crab.col, crab.row, crab.shellCol, crab.shellRow))
            end
        end
    end
end
...
function Battle.CheckEndCondition(state)
    ...
    if state.hero.hp <= 0 then
        ...
        if Skills.Level(state.skills, "dawn_herald") >= 1 and not state.hero._dawnUsed then
            state.hero._dawnUsed = true
