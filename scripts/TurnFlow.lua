...
function TurnFlow.NextLevel()
    ...
    G.battle.turn = G.battle.turn  -- 保持回合数延续
    G.battle.combo = 0
    G.battle.maxCombo = 0
...
-- 阶段1：道具教学（首次） → ExecuteComboAndProceed
function TurnFlow.FinishExecution()
    TurnFlow.ClearPlan()

    -- 关闭可能还在显示的多格跳跃教程（避免两个教程同时出现）
    if G.multiHopSpotlightActive then
        G.multiHopSpotlightActive = false
        G.multiHopSpotlightJump = nil
    ...
    -- === 预检查：是否有首次道具教学需要在效果前弹出 ===
    local firstBoardItem = nil
    if G.battle and not G.playerData.itemTutorialSeen and G.battle.items then
        for _, item in ipairs(G.battle.items) do
            if item and item.col and item.row then
                firstBoardItem = item
                break
            end
        end
    ...
    -- 道具教学 → 连击效果
    if firstBoardItem then
        local ok, err = pcall(GameUI.ShowItemTutorial, firstBoardItem, function()
            TurnFlow.ExecuteComboAndProceed()
        end)
        if ok then
            G.playerData.itemTutorialSeen = true
            PlayerData.Save(G.playerData)
        else
            log:Write(LOG_ERROR, "[ItemTutorial] " .. tostring(err))
            TurnFlow.ExecuteComboAndProceed()
        end
    else
        TurnFlow.ExecuteComboAndProceed()
...
function TurnFlow.ShowChapterClear(chapter)
    ...
    -- 是否有下一章
    -- 注意：第4章（六芒对抗）仍是 WIP，暂不开放，故上限写死为 3
    -- 开放第4章时将此处改为 4，并同步更新 MenuPages.lua 的章节选择逻辑
    local maxChapter = 3
    ...
    if G.battle then
        ...
        if phase ~= "WIN" and phase ~= "LOSE" and phase ~= "COMBO_REWARD_WAIT" and phase ~= "PLAYER_EXECUTE" then
            ...
            if not checkOk then
                ...
                result = nil
                if G.battle.hero and G.battle.hero.hp <= 0 then
                    result = "LOSE"
