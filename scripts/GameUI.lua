...
--- 道具教程（聚光灯高亮样式）
--- 遮罩和高亮由 BoardWidget 的 NanoVG 渲染完成，这里只放点击拦截层
function GameUI.ShowItemTutorial(firstItem, onClose)
    AM.PlaySFX("ui_popup_open")

    -- 移除旧弹窗
    if G.itemTutorialPopup then
        G.itemTutorialPopup:Remove()
        G.itemTutorialPopup = nil
    end

    -- 设置标志，BoardWidget 渲染时会绘制聚光灯遮罩 + 提示文字
    G.itemSpotlightActive = {
        col = firstItem and firstItem.col or nil,
        row = firstItem and firstItem.row or nil,
        itemType = firstItem and firstItem.type or nil,
    }

    -- 透明全屏点击拦截层，点击任意位置关闭（按钮在 BoardWidget NanoVG 中绘制）
    G.itemTutorialPopup = UI.Panel {
        position = "absolute",
        top = 0, left = 0, right = 0, bottom = 0,
        zIndex = 810,
        onClick = function(self)
            AM.PlaySFX("ui_popup_close")
            G.itemSpotlightActive = nil
            if G.itemTutorialPopup then
                G.itemTutorialPopup:SetVisible(false)
            end
            if onClose then onClose() end
        end,
    }

    UI.GetRoot():AddChild(G.itemTutorialPopup)
end
