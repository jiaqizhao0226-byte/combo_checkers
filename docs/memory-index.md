...
- `.cli/`、`logs/` 是运行时/诊断状态，不随普通功能提交；`screenshots/` 用于保存并同步验证截图。

## 最近变更
- 2026-08-07：按用户确认上传 `screenshots/` 中 4 张竖屏 UI 缩放验证图，合计约 17MB；私有记忆、`.cli/` 和 `logs/` 继续排除。
- 2026-08-07：核对目标仓库与工作树；项目源码已与 `origin/master` 同步；补充私有记忆和本地截图忽略规则，继续排除 `.cli/` 和 `logs/` 运行时状态。
